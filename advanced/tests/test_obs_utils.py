...
def test_parse_start_and_end(capsys) -> None:
    ...
    assert start == 70
    assert end == 100
    start, end = parse_start_and_end(-110, 0, info, False)
    ...
    start, end = parse_start_and_end(30, 40, info, False)
    assert start is None
    assert end is None
    captured = capsys.readouterr()
    ...
    assert start == 50
    assert end == 80
    start, end = parse_start_and_end(90, 80, info, False)
    assert start is None
    assert end is None
    captured = capsys.readouterr()
    assert captured.out == "{'text/markdown': '#### Invalid `start`:'}\n{'text/markdown': 'Start height (90) is higher than end height (80)'}\n"
    start, end = parse_start_and_end('wrong', 100, info, False)
    assert start is None
    assert end is None
    captured = capsys.readouterr()
    assert captured.out == "{'text/markdown': '#### Invalid `start`:'}\n{'text/markdown': 'Start height is not a valid integer number'}\n"
    start, end = parse_start_and_end(70, 'wrong', info, False)
    assert start is None
    assert end is None
    captured = capsys.readouterr()
    assert captured.out == "{'text/markdown': '#### Invalid `end`:'}\n{'text/markdown': 'End height is not a valid integer number'}\n"
    start, end = parse_start_and_end(70, -10, info, False)
    assert start is None
    assert end is None
    captured = capsys.readouterr()
