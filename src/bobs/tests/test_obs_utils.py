...
from bobs.obs.obs_utils import parse_start_and_end, check_memory
...
def test_parse_start_and_end(capsys) -> None:
    ...
    with raises(TypeError):
        parse_start_and_end('wrong', 100, info, False)  # type: ignore
        captured = capsys.readouterr()
        assert captured.out == "{'text/markdown': '#### Invalid `start`:'}\n{'text/markdown': 'Start height is not a valid integer number'}\n"
    with raises(TypeError):
        parse_start_and_end(70, 'wrong', info, False)  # type: ignore
        captured = capsys.readouterr()
...
def test_fuzz_parse_start_and_end(start, end, force):
    try:
        parse_start_and_end(start=start, end=end, info=info, force=force)
...
@given(memory_limit=st.one_of(st.none(), st.integers()))
def test_fuzz_check_memory(memory_limit):
    try:
        check_memory(memory_limit=memory_limit)
    except MemoryError:
        reject()
